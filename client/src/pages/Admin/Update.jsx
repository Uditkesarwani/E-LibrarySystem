...
const Update = () => {
  ...
  const [book, setBook] = useState([])
   const [form, setForm] = useState({
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        const response = await axios.post(`http://localhost:4000/admin/update-book/${data._id}`, formData, {
          headers: {
 ...
 navigate('/viewBook')
   ...
   const fetchUpdata = async()=>{
    const response = await axios.get(`http://localhost:4000/admin/update-book/${data._id}`)
    if(response.data.success){
